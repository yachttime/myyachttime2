...
import { Plus, CreditCard as Edit2, AlertCircle, Package, TrendingUp, TrendingDown, Search, Camera, X, Printer } from 'lucide-react';
              ...
              <div className="relative">
                ...
                {showPartLookup && partLookupResults.length > 0 && (
                  <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-white border border-gray-300 rounded-lg shadow-xl max-h-64 overflow-y-auto">
                    <div className="px-3 py-2 text-xs font-semibold text-gray-500 bg-gray-50 border-b border-gray-200 sticky top-0 flex items-center justify-between">
                      <span>Select to auto-fill from database</span>
                      <button
                        type="button"
                        onMouseDown={() => { setShowPartLookup(false); setPartLookupResults([]); }}
                        className="text-gray-400 hover:text-gray-700 ml-2"
                        title="Dismiss and enter as new part"
                      >
                    ...
                    <button
                      type="button"
                      onMouseDown={() => { setShowPartLookup(false); setPartLookupResults([]); }}
                      className="w-full text-left px-3 py-2.5 hover:bg-gray-50 border-t border-gray-200 text-xs text-gray-500 flex items-center gap-2"
                    >
                      <X className="w-3 h-3" />
                      Dismiss — enter "{formData.part_number}" as a new part
                    </button>
                  </div>
                )}
              </div>

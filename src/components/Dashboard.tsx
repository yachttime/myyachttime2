...
export const Dashboard = ({ onNavigate }: DashboardProps) => {
  ...
  const handleSendDepositEmail = async () => {
    ...
    setSendingEmail(true);
    try {
      if (!selectedRepairForDepositEmail.deposit_payment_link_url) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-white flex">
      ...
      <aside className={`w-64 bg-slate-900/90 border-r border-slate-700 flex flex-col fixed h-full z-40 transition-transform duration-300 ${
        ...
        <div className="p-6 border-b border-slate-700">
          ...
          <div className="text-xs font-semibold text-amber-500 bg-amber-500/10 px-2 py-1 rounded inline-block">v2026.02.05.A</div>
        </div>
                    ...
                    {!isWithinBookingPeriod(activeBooking) && (
                      <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 mb-4">
                        <p className="text-blue-200 text-sm text-center font-medium">
                          Check-in and check-out buttons will become active on {formatDate(activeBooking.start_date)} at {formatTime(activeBooking.start_date)}.
                        </p>
                      </div>
                    )}
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    try {
                                      ...
                                      if (error) throw error;
                                      await loadCustomers();
                                      setRepairForm({...repairForm, customer_id: data.id, vessel_id: ''});
                                      setSelectedCustomerId(data.id);
                                      await loadCustomerVessels(data.id);
                                      setShowQuickAddCustomer(false);
                                      setQuickCustomerForm({ customer_type: 'individual', first_name: '', last_name: '', business_name: '', email: '', phone: '' });
                                    } catch (error: any) {
                                    ...
                                    <button
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          setQuickVesselForm({ vessel_name: '', manufacturer: '', model: '', year: '' });
                                        } catch (error: any) {
                                  ...
                                  {request.estimate_email_sent_at && (
                                    <div className="mt-3 bg-slate-900/50 rounded-lg p-3 border border-slate-700">
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        ...
                                        {request.email_delivered_at && !request.email_opened_at && !request.email_clicked_at && (
                                          <span className="bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                                            <CheckCircle className="w-3 h-3" />
                                            Email Delivered
                                          </span>
                                        )}
                                        {request.email_opened_at && !request.email_clicked_at && (
                                          <span className="bg-indigo-500/20 text-indigo-400 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                                            <Mail className="w-3 h-3" />
                                            Email Opened
                                          </span>
                                        )}
                                        {request.email_clicked_at && (
                                          <span className="bg-purple-500/20 text-purple-400 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                                            <MousePointer className="w-3 h-3" />
                                            Link Clicked
                                          </span>
                                        )}
                                        {request.email_bounced_at && (
                                          <span className="bg-red-500/20 text-red-400 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                                            <AlertCircle className="w-3 h-3" />
                                            Email Bounced
                                          </span>
                                      ...
                                      <div className="space-y-1 mt-3 pt-3 border-t border-slate-700">
                                        ...
                                        {request.email_delivered_at && (
                                          <div className="flex items-center gap-2 text-xs text-emerald-400">
                                            <CheckCircle className="w-3 h-3" />
                                            <span>Delivered: {new Date(request.email_delivered_at).toLocaleDateString()} at {new Date(request.email_delivered_at).toLocaleTimeString()}</span>
                                          </div>
                                        )}
                                        {request.email_opened_at && (
                                          <div className="flex items-center gap-2 text-xs text-cyan-400">
                                            <Mail className="w-3 h-3" />
                                            <span>Opened: {new Date(request.email_opened_at).toLocaleDateString()} at {new Date(request.email_opened_at).toLocaleTimeString()}</span>
                                            {request.email_open_count > 1 && (
                                              <span className="bg-cyan-500/20 text-cyan-400 px-2 py-0.5 rounded text-xs">
                                                {request.email_open_count}x
                                              </span>
                                            )}
                                          </div>
                                        )}
                                        {request.email_clicked_at && (
                                          <div className="flex items-center gap-2 text-xs text-teal-400">
                                            <MousePointer className="w-3 h-3" />
                                            <span>Clicked: {new Date(request.email_clicked_at).toLocaleDateString()} at {new Date(request.email_clicked_at).toLocaleTimeString()}</span>
                                            {request.email_click_count > 1 && (
                                              <span className="bg-teal-500/20 text-teal-400 px-2 py-0.5 rounded text-xs">
                                                {request.email_click_count}x
                                              </span>
                                            )}
                                          </div>
                                        )}
                                        {request.email_bounced_at && (
                                          <div className="flex items-center gap-2 text-xs text-red-400">
                                            <AlertCircle className="w-3 h-3" />
                                            <span>Bounced: {new Date(request.email_bounced_at).toLocaleDateString()} at {new Date(request.email_bounced_at).toLocaleTimeString()}</span>
                                          </div>
                                        )}
                                      </div>
                                      {canManageYacht(effectiveRole) && (
                                              ...
                                              <div className="space-y-1">
                                                ...
                                                {request.deposit_email_opened_at && (
                                                  <div className="flex items-center gap-2 text-xs text-green-400">
                                                    <Eye className="w-3 h-3" />
                                                    <span>Opened: {new Date(request.deposit_email_opened_at).toLocaleDateString()} at {new Date(request.deposit_email_opened_at).toLocaleTimeString()}</span>
                                                  </div>
                                                )}
                                                {request.deposit_email_clicked_at && (
                                                  <div className="flex items-center gap-2 text-xs text-cyan-400">
                                                    <MousePointer className="w-3 h-3" />
                                  ...
                                  {!isExpired && (
                                    <>
                                      <button
                                        ...
                                        className="px-3 py-2 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-semibold transition-all"
                                      >
                                        Copy
                                      </button>
                                      <button
                                        onClick={() => handleOpenDepositEmailModal(selectedRepairForDeposit)}
                                        className="px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded text-sm font-semibold transition-all flex items-center gap-1"
                                      >
                                        <Mail className="w-3 h-3" />
                                        Email
                                      </button>
      ...
      {showDepositEmailModal && selectedRepairForDepositEmail && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl shadow-2xl w-full max-w-md border border-slate-700">
            <div className="p-6 border-b border-slate-700">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  ...
                  Send Deposit Request via Email
                </h3>
                <button
            ...
            <div className="p-6 space-y-4">
              <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                ...
                {selectedRepairForDepositEmail.deposit_amount && (
                  <p className="text-cyan-400 text-lg font-bold mt-1">
                    ${parseFloat(selectedRepairForDepositEmail.deposit_amount).toFixed(2)}
                  </p>
                )}
              </div>

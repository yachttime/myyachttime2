...
export function PrintableVesselAgreement({ agreement, onClose }: PrintableVesselAgreementProps) {
  ...
  const totalAmount = (agreement.annual_fee ?? 8000) + ((agreement.season_trips || 0) + (agreement.off_season_trips || 0)) * (agreement.per_trip_fee || 350);
      ...
      <div className="print-wrapper">
        ...
        <div className="mb-6">
          ...
          <div className="text-xs space-y-1.5 leading-relaxed">
            ...
            <p><strong>2.</strong> Boatco, on behalf of each Equity Owner and vessel, on or before <strong className="border-b border-black px-2">{agreement.contract_date || '________________'}</strong> the contract Start date, shall pay an annual management fee of <strong className="border-b border-black px-2">${(agreement.annual_fee ?? 8000).toFixed(2)}</strong> ("Annual Fee"), in addition to the Annual Fee the Boatco agrees to pay the Liaison $350.00 per turn to do a full 2-hour mechanical inspection of the boat, and will provide the Boatco a report with the findings. The estimated number of trips for this agreement will be <strong className="border-b border-black px-2">{(agreement.season_trips || 0) + (agreement.off_season_trips || 0)}</strong> this will include <strong className="border-b border-black px-2">{agreement.season_trips || 0}</strong> in season trips and <strong className="border-b border-black px-2">{agreement.off_season_trips || 0}</strong> off-season trips at $350.00 each totaling <strong className="border-b border-black px-2">${(((agreement.season_trips || 0) + (agreement.off_season_trips || 0)) * (agreement.per_trip_fee || 350)).toFixed(2)}</strong>. For a grand total of <strong className="border-b border-black px-2">${((agreement.annual_fee ?? 8000) + (((agreement.season_trips || 0) + (agreement.off_season_trips || 0)) * (agreement.per_trip_fee || 350))).toFixed(2)}</strong> This is a one-time fee per season payable to the Liaison, the Boatco understands that repairs and maintenance will be additional.</p>

...
export default function CatalogSearchDropdown({ equipmentType, onSelect, placeholder = 'Pick from catalog...', className }: CatalogSearchDropdownProps) {
  ...
  const selectResult = (row: CatalogRow) => {
    const entry: Partial<EngineGenFormEntry> & { catalog_id: string } = {
      ...
      include_oil_filter: row.include_oil_filter !== false,
      include_fuel_filter: row.include_fuel_filter !== false,
      include_impeller: row.include_impeller !== false,
      include_belt1: row.include_belt1 !== false,
      include_belt2: row.include_belt2 !== false,
      include_spark_plug: row.include_spark_plug !== false,
      include_distributor_cap: row.include_distributor_cap !== false,
      include_rotor: row.include_rotor !== false,
      include_plug_wires: row.include_plug_wires !== false,
      include_oil_weight: row.include_oil_weight !== false,
      include_oil_quantity: row.include_oil_quantity !== false,
      include_oil_filter_alt1: row.include_oil_filter_alt1 !== false,
      include_oil_filter_alt2: row.include_oil_filter_alt2 !== false,
      include_fuel_filter_alt1: row.include_fuel_filter_alt1 !== false,
      include_fuel_filter_alt2: row.include_fuel_filter_alt2 !== false,
      include_impeller_alt1: row.include_impeller_alt1 !== false,
      include_impeller_alt2: row.include_impeller_alt2 !== false,
      include_belt1_alt1: row.include_belt1_alt1 !== false,
      include_belt1_alt2: row.include_belt1_alt2 !== false,
      include_belt2_alt1: row.include_belt2_alt1 !== false,
      include_belt2_alt2: row.include_belt2_alt2 !== false,
    };
    onSelect(entry);
    setQuery('');
    setShowDropdown(false);

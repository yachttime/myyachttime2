/*
# Fix approve_estimate to set status to 'converted' instead of 'approved'

1. Problem
   - The approve_estimate function sets the estimate status to 'approved'
   - The Estimates screen filters out estimates with status 'converted'
   - This means approved estimates still appear in the estimates list after being converted to work orders

2. Fix
   - Change the status update from 'approved' to 'converted' so the estimate disappears from the active list
   - Keep approved_by and approved_at fields populated for audit trail
*/

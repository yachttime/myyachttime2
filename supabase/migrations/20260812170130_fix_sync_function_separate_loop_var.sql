/*
# Fix sync function: use separate variables for invoice record and loop cursor

## Problem
The sync_invoice_line_items_from_work_order function reuses v_invoice_record
as both the fetched invoice record AND the loop cursor variable for iterating
work order line items. After the loop runs, v_invoice_record no longer holds
the invoice data — it holds the last line item — so accessing
v_invoice_record.discount_amount fails.

## Fix
Use a separate variable v_li for the loop cursor, keeping v_invoice_record
intact for the invoice header fields needed in the totals recalculation.
*/

/*
  # Fix approve_estimate to carry repair request deposit to work order

  ## Problem
  When an estimate linked to a repair request is approved, the work order
  was created without the repair request's paid deposit. This caused invoices
  converted from those work orders to show the full amount with no deposit applied.

  ## Changes
  - `approve_estimate`: After creating the work order, check if any repair request
    is linked to this estimate and has a paid deposit. If so, copy the deposit
    amount and status to the new work order.
*/

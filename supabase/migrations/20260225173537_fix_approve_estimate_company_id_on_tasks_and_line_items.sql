/*
  # Fix approve_estimate to propagate company_id to work_order_tasks and work_order_line_items

  ## Summary
  When an estimate is approved and a work order is created, the INSERT statements for
  work_order_tasks and work_order_line_items did not include company_id. RLS SELECT policies
  for staff/mechanic roles require company_id to match, so those rows were invisible when
  opening a work order.

  ## Changes
  - work_order_tasks INSERT now includes company_id = v_company_id
  - work_order_line_items INSERT now includes company_id = v_company_id
*/

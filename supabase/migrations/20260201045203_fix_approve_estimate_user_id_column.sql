/*
  # Fix Approve/Deny Estimate Functions - User ID Column

  1. Changes
    - Fix approve_estimate to check user_profiles.user_id instead of user_profiles.id
    - Fix deny_estimate to check user_profiles.user_id instead of user_profiles.id
    - The frontend passes auth.users.id which maps to user_profiles.user_id
  
  2. Security
    - Maintains same permission logic
    - Staff, mechanic, master OR manager with can_approve_repairs = true
*/

-- Fix approve_estimate function
...
BEGIN
  ...
  -- Update estimate status to approved (use profile id for approved_by)
  UPDATE estimates
  SET 
    status = 'approved',
    approved_by = v_user_profile_id,
    approved_at = now(),
    updated_at = now()
  WHERE id = p_estimate_id;
  ...
  -- Create work order (use profile id for created_by)
  ...
  -- Process inventory deductions (use profile id)
  SELECT process_estimate_inventory_deduction(p_estimate_id, v_user_profile_id)
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'You do not have permission to deny estimates';
  END IF;

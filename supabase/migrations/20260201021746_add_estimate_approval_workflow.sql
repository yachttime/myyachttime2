/*
  ...
  1. Functions
    - `approve_estimate` - Approves an estimate and converts it to a work order
      - Updates estimate status to 'approved'
      - Sets approved_by and approved_at fields
      - Creates a new work order record
      - Deducts parts from inventory
      - Returns work order ID and any low stock alerts
    
    - `deny_estimate` - Denies/rejects an estimate
      - Updates estimate status to 'rejected'
      - Does NOT adjust inventory
  
  2. Security
    - Only staff, mechanic, and master roles can approve/deny estimates
    - Validates estimate exists and is in correct status
*/

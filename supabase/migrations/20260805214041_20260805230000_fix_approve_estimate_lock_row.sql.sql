/*
  # Fix approve_estimate to lock the estimate row and prevent race conditions

  ## Problem
  The approve_estimate function checks estimate status with a plain SELECT, then
  updates it later. If two calls race, both can pass the status check before either
  sets status to 'converted', resulting in two work orders being created.

  ## Fix
  Use SELECT ... FOR UPDATE to lock the estimate row at the start of the function,
  so concurrent calls block until the first one commits.
*/
...
-- Lock the estimate row to prevent concurrent approval

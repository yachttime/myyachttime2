/*
  # Enforce Company Isolation for All Users Including Masters

  1. Problem
    - Many RLS policies use `is_master_user() OR (company_id = get_user_company_id())`
    - This allows master users to bypass company isolation and see ALL data
    - Master in "test company" could see "AZ Marine" data
    
  2. Solution
    - Remove `is_master_user()` bypass from all company-scoped policies
    - All users (including masters) must match company_id
    - Masters have elevated permissions WITHIN their company only
    
  3. Security
    - Complete company data isolation
    - No cross-company access for any role
    - Masters can see company list for switching context (frontend)
    - But all data queries are filtered by company_id via RLS
*/

-- Fix yachts table policies
...
CREATE POLICY "Users can view their company yachts"
  ON yachts FOR SELECT
  TO authenticated
  USING (company_id = get_user_company_id());

CREATE POLICY "Staff can update their company yachts"
...
CREATE POLICY "Staff can delete their company yachts"
...
CREATE POLICY "Users can view their company booking owners"
  ON yacht_booking_owners FOR SELECT
  TO authenticated
  USING (company_id = get_user_company_id());

CREATE POLICY "Staff can update their company booking owners"
  ON yacht_booking_owners FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company booking owners"
...
CREATE POLICY "Staff can update their company work orders"
  ON work_orders FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company work orders"
...
CREATE POLICY "Users can view their company work order tasks"
  ON work_order_tasks FOR SELECT
  TO authenticated
  USING (company_id = get_user_company_id());

CREATE POLICY "Staff can update their company work order tasks"
  ON work_order_tasks FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company work order tasks"
...
CREATE POLICY "Staff can update their company work order line items"
  ON work_order_line_items FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company work order line items"
...
CREATE POLICY "Users can view their company task assignments"
  ON work_order_task_assignments FOR SELECT
  TO authenticated
  USING (company_id = get_user_company_id());

CREATE POLICY "Staff can update their company task assignments"
  ON work_order_task_assignments FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company task assignments"
...
CREATE POLICY "Users can view their company work order time entries"
  ON work_order_time_entries FOR SELECT
  TO authenticated
  USING (company_id = get_user_company_id());

CREATE POLICY "Staff can update their company work order time entries"
  ON work_order_time_entries FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company work order time entries"
...
CREATE POLICY "Users can view their company estimates"
  ON estimates FOR SELECT
  TO authenticated
  USING (company_id = get_user_company_id());

CREATE POLICY "Staff can update their company estimates"
  ON estimates FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company estimates"
...
CREATE POLICY "Staff can update their company customers"
  ON customers FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company customers"
...
CREATE POLICY "Users can view their company customer vessels"
  ON customer_vessels FOR SELECT
  TO authenticated
  USING (company_id = get_user_company_id());

CREATE POLICY "Staff can update their company customer vessels"
  ON customer_vessels FOR UPDATE
  TO authenticated
  USING (company_id = get_user_company_id() AND is_staff())
  WITH CHECK (company_id = get_user_company_id() AND is_staff());

CREATE POLICY "Staff can delete their company customer vessels"
...
CREATE POLICY "Staff can delete their company maintenance requests"

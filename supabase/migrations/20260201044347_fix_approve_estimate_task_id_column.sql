/*
  # Fix approve_estimate function to use correct column name

  1. Changes
    - Update approve_estimate function to use `task_id` instead of `estimate_task_id`
    - The estimate_line_items table has a column named `task_id`, not `estimate_task_id`
  
  2. Security
    - No changes to security policies
*/

-- Function to approve estimate and convert to work order (FIXED VERSION)
...
BEGIN
  ...
  -- Copy estimate line items to work order line items (FIXED: using task_id instead of estimate_task_id)

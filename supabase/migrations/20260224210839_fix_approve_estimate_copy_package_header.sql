/*
  # Fix approve_estimate to copy package_header from estimate_line_items

  ## Summary
  Updates the approve_estimate function so that when an estimate is converted
  to a work order, the `package_header` value on each line item is preserved.

  ## Changes
  - `approve_estimate` function: adds `package_header` to the INSERT ... SELECT
    that copies estimate_line_items into work_order_line_items
*/
...
BEGIN
  ...
  -- Copy estimate line items to work order line items (including package_header)

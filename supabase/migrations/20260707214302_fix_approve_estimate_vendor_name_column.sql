/*
# Fix approve_estimate vendor column reference

## Problem
The `approve_estimate` function references `v.name` from the `vendors` table,
but the actual column is `vendor_name`. This causes "column v.name does not exist"
when approving an estimate that has parts with a vendor assigned.

## Fix
Change `v.name` to `v.vendor_name` in the purchase order creation query within
`approve_estimate`.
*/

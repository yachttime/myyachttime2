-- Fix: convert_work_order_to_invoice was missing trip_inspection_id in the INSERT,
-- causing trip inspection reports to disappear from invoices after conversion.
-- This was dropped when migration 20260801000003 rewrote the function for totals recalculation.

-- Step 1: Backfill existing invoices that lost their trip_inspection_id
UPDATE estimating_invoices ei
SET trip_inspection_id = wo.trip_inspection_id,
    updated_at = now()
FROM work_orders wo
WHERE ei.work_order_id = wo.id
  AND ei.trip_inspection_id IS NULL
  AND wo.trip_inspection_id IS NOT NULL;

-- Step 2: Recreate the function with trip_inspection_id carried from work order to invoice

/*
# Fix approve_estimate purchase_orders column name

## Problem
The `approve_estimate` function references `vendor_contact` but the actual column
in `purchase_orders` is `vendor_contact_name`. This causes "column vendor_contact
of relation purchase_orders does not exist" when approving estimates with vendor parts.

## Fix
Change `vendor_contact` to `vendor_contact_name` in the INSERT INTO purchase_orders statement.
*/

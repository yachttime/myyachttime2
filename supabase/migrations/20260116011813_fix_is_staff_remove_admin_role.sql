/*
  # Fix is_staff Function - Remove Invalid 'admin' Role

  1. Problem
    - The is_staff() function checks for 'admin' role which doesn't exist in the user_role enum
    - Valid roles are: owner, manager, staff, mechanic
    - This causes all queries to fail with "invalid input value for enum user_role: admin"
  
  2. Solution
    - Update is_staff() functions to only check valid roles: staff, manager, mechanic
    - Remove 'admin' from the role check
  
  3. Impact
    - All database queries will work correctly
    - Staff users (staff, manager, mechanic) will be properly recognized
*/

-- Drop and recreate the functions with correct roles

/*
# Fix process_estimate_inventory_deduction call in approve_estimate

1. Problem
   - The approve_estimate function calls process_estimate_inventory_deduction with wrong argument order
   - Current call: (p_estimate_id, v_work_order_id, v_yacht_name, p_user_id)
   - Correct signature: (p_estimate_id, p_user_id, p_work_order_id, p_company_id)
   - v_yacht_name (text) was passed where p_company_id (uuid) is expected
   - v_work_order_id was in the p_user_id position

2. Fix
   - Reorder arguments to match function signature
   - Replace v_yacht_name with v_company_id (which is already available in the function)
*/
...
-- Fixed: correct argument order (p_estimate_id, p_user_id, p_work_order_id, p_company_id)

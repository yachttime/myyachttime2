/*
  # Fix approve_estimate: link repair_request.work_order_id and sync estimate deposit columns

  ## Problem
  1. When approve_estimate creates a work order from an estimate that has a linked
     repair_request, it never sets repair_requests.work_order_id = new work order id.
     This breaks the trigger that syncs repair request deposits retroactively.
  2. The new repair_request_deposit_* columns on estimates are never populated at
     approval time when a deposit was already paid.

  ## Changes
  - After creating work order, set repair_requests.work_order_id = v_work_order_id
    for the linked repair request (if any).
  - Also populate the new estimate deposit sync columns if deposit is already paid.
*/
...
v_estimate.total_amount, v_estimate.notes, v_estimate.customer_notes, v_estimate.work_title, v_company_id, p_user_id
)

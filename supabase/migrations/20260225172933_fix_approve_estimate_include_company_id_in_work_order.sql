/*
  # Fix approve_estimate function to include company_id in work_order INSERT

  ## Summary
  The approve_estimate function fetches v_company_id from the estimate but does not
  pass it to the INSERT INTO work_orders statement. This causes new work orders to
  have company_id = NULL, making them invisible to staff/mechanic users filtered by
  company RLS policies.

  ## Changes
  - Adds company_id to the INSERT INTO work_orders column list and VALUES
  - Also adds work_title to carry over from the estimate to the work order

  ## Notes
  - v_company_id is already declared and populated in the function
  - This is a DROP and recreate of the function to update the INSERT statement
*/

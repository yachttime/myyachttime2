-- ============================================================
-- Revert: Change billing due dates and payment link expirations
-- back to 30 days (undoing the 48-hour change)
-- ============================================================

-- 1. Replace convert_work_order_to_invoice with 30-day due date
...
-- ============================================================
-- 2. Revert existing unpaid estimating invoices back to 30-day due dates
-- ============================================================
UPDATE estimating_invoices
SET due_date = invoice_date + INTERVAL '30 days'
WHERE payment_status NOT IN ('paid')
  AND due_date != invoice_date + INTERVAL '30 days';

-- ============================================================
-- 3. Revert pending payment link expirations back to 30 days from now
-- ============================================================

-- Yacht invoices: pending payment links
UPDATE yacht_invoices
SET payment_link_expires_at = NOW() + INTERVAL '30 days'
...
UPDATE estimating_invoices
SET final_payment_link_expires_at = NOW() + INTERVAL '30 days'
WHERE final_payment_link_expires_at IS NOT NULL
  AND payment_status NOT IN ('paid', 'processing');

-- Repair requests: pending deposit links
UPDATE repair_requests
SET deposit_link_expires_at = NOW() + INTERVAL '30 days'

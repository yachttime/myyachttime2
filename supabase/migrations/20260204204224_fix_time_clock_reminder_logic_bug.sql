/*
  # Fix Time Clock Reminder Logic Bug

  1. Problem
    - The current function uses: `start_time <= (check_time - 10 minutes)`
    - This causes time wraparound issues at midnight
    - Example: At 00:00, it checks if 08:00 <= 23:50, which is TRUE
    - Result: Reminders sent at midnight instead of 8:10am

  2. Solution
    - Check if start_time is within a 20-minute window: between (check_time - 20 min) and (check_time - 10 min)
    - This ensures reminders are sent only during the correct time window
...
BEGIN
  ...
  WITH staff_on_duty AS (
    ...
    WHERE ss.is_working_day = true
      ...
      -- This creates a 10-minute window for sending reminders

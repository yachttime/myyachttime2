/*
  # Change All Billing Due Dates and Payment Link Expirations to 48 Hours

  ## Purpose
  Business rule: all bills are due within 48 hours. Previously the system used
  30-day payment link expirations and "due on receipt" (same-day) invoice due dates.
  This migration aligns everything to a 48-hour (2-day) standard.

  ## Changes

  ### 1. Update convert_work_order_to_invoice function
  - Changed due_date from CURRENT_DATE + INTERVAL '30 days' to CURRENT_DATE + INTERVAL '2 days'
  - All other logic unchanged (financial recalculation, deposit, payment, line items)

  ### 2. Update existing unpaid estimating invoices
  - Set due_date = invoice_date + INTERVAL '2 days' for all unpaid invoices

  ### 3. Update existing pending payment link expirations to 48 hours from now
  - yacht_invoices: payment_link_expires_at
  - estimating_invoices: final_payment_link_expires_at
  - repair_requests: deposit_link_expires_at
  - work_orders: deposit_link_expires_at

  ## Important Notes
  1. Only pending/unpaid records are updated — paid invoices are untouched.
  2. The convert_work_order_to_invoice function is replaced entirely with the latest
     version (from 20260801000003) with only the due_date INTERVAL changed.
  3. yacht_invoices has no due_date column — only estimating_invoices does.
*/

-- ============================================================
-- 1. Replace convert_work_order_to_invoice with 48-hour due date
-- ============================================================
...
-- ============================================================
-- 2. Update existing unpaid estimating invoices: due_date = invoice_date + 2 days
-- ============================================================
UPDATE estimating_invoices
SET due_date = invoice_date + INTERVAL '2 days'
WHERE payment_status NOT IN ('paid')
  AND due_date != invoice_date + INTERVAL '2 days';

-- ============================================================
-- 3. Update pending payment link expirations to 48 hours from now
-- ============================================================

-- Yacht invoices: pending payment links
UPDATE yacht_invoices
SET payment_link_expires_at = NOW() + INTERVAL '2 days'
WHERE payment_link_expires_at IS NOT NULL
  AND payment_status NOT IN ('paid', 'processing');

-- Estimating invoices: pending final payment links
UPDATE estimating_invoices
SET final_payment_link_expires_at = NOW() + INTERVAL '2 days'
WHERE final_payment_link_expires_at IS NOT NULL
  AND payment_status NOT IN ('paid', 'processing');

-- Repair requests: pending deposit links
UPDATE repair_requests
SET deposit_link_expires_at = NOW() + INTERVAL '2 days'
WHERE deposit_link_expires_at IS NOT NULL
  AND deposit_payment_status = 'pending';

-- Work orders: pending deposit links
UPDATE work_orders
SET deposit_link_expires_at = NOW() + INTERVAL '2 days'
WHERE deposit_link_expires_at IS NOT NULL
  AND deposit_payment_status = 'pending';
